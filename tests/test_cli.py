...
def test_parser_requires_modelopt_and_accepts_bounds():
    p = cli.build_arg_parser()
    ns = p.parse_args(["scan", "--modelopt", "/x", "--min", "4.48.0", "--max", "4.52.0"])
    assert ns.modelopt == "/x" and ns.min == "4.48.0" and ns.max == "4.52.0"
...
def test_main_end_to_end_with_monkeypatched_seams(tmp_path, monkeypatch):
    ...
    out = tmp_path / "report"
    rc = cli.main(["scan", "--modelopt", "/x", "--min", "4.48.0", "--max", "4.50.0",
                   "--out", str(out)])
    assert rc == 0
